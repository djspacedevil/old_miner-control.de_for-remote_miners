...
class session {
	...
	function write($id, $data) {
		...
		if(!isset($this->w_stmt)) {
			$this->w_stmt = $this->db->prepare("REPLACE INTO sessions (`id`, `user_id`, `set_time`, `data`, `session_key`, `ip`) VALUES (?, ?, ?, ?, ?, ?)");
		}
 
		$this->w_stmt->bind_param('siisss', $id, $user_id, $time, $data, $key, $ip);
		$this->w_stmt->execute();
		return true;
	}
	...
	private function encrypt($data, $key) {
		$salt = 'DSsdgrsaRASGar34512345RGAGaaophjäjopa<t3wv gq4bt98n§$%§aö giavrn';  //USE your own SALT HERE
